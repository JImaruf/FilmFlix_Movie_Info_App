    ...
    return GetBuilder<HomeController>(builder: (ctrl) {
      return Scaffold(
        appBar: AppBar(
          ...
          centerTitle: true,
          title: const Text(
            'FILMFLIX', style: TextStyle(color: Colors.red, fontSize: 24),),
        ),
            ...
            child
              ...
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 10,),
                  FutureBuilder(future: ctrl.getTrending(),
                    builder: (context, snapshot) {
                    if(ctrl.trendingMovieList.isNotEmpty)
                      {
                        print("snanpshot");
                        return TrendingMovies(trendingList: ctrl.trendingMovieList,);
                      }
                    else
                      {
                       return Text('Loading');
                  ...
                  SizedBox(height: 32,),
                  Text('Top Rated Movies', style: TextStyle(fontSize: 20)),
                  SizedBox(height: 16,),
                  FutureBuilder(future: ctrl.topRated(),
                    builder: (context, snapshot) {
                      if(ctrl.topRatedMovieList.isNotEmpty)
                      {
                        print("snanpshot");
                        return MoviesSlider(movieList: ctrl.topRatedMovieList,);
                      }
                      else
                      {
                        return Text('Loading');
                  ...
                  SizedBox(height: 16,),
                  FutureBuilder(future: ctrl.Upcoming(),
                    builder: (context, snapshot) {
                      if(ctrl.upComingMovieList.isNotEmpty)
                      {
                        print("snanpshot");
                        return MoviesSlider(movieList: ctrl.upComingMovieList,);
                      }
                      else
                      {
                        return Text('Loading');
